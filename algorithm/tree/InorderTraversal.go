...
//   c). go to step 3
func inorderTraversalIterative(root *base.TreeNode) []int {
	if root == nil {
		return []int{}
	}
...
func inorderTraversalRecursive(root *base.TreeNode) []int {
	ans := []int{}
	inorderTraversalRecursiveCore(root, &ans)
	return ans
}

func inorderTraversalRecursiveCore(node *base.TreeNode, ans *[]int) {
	if node == nil {
		return
	}
